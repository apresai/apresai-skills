...
| **[codex-br](./plugins/codex-br/)** | `/codex-br task` · `/codex-br review` · `/codex-br adversarial-review` | Run **OpenAI Codex on Amazon Bedrock** (`openai.gpt-5.6-sol`, provider `amazon-bedrock`) instead of the default ChatGPT backend, via a `br` Codex profile. Delegate tasks, run the built-in reviewer, or run a steerable adversarial challenge review, all at max reasoning effort (`ultra`), output returned verbatim. The Bedrock twin of OpenAI's `codex:*` plugin; includes one-time setup for the profile, the `~/.codex/.env` token pin, and the `codex-br` shell alias. |
