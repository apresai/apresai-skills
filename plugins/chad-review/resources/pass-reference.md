...
### `[types/mirror]` Reverse reconciliation via contract-mirror.sh

The forward check above asks "is the generated artifact fresh". This one asks
the reverse: "did anything handwritten survive that the generator now covers".
The Swift note above ("handwritten types duplicating generated ones") named the
defect without a mechanism; `contract-mirror.sh` is the mechanism, and it is
never re-implemented with ad-hoc greps.

Record grammar (tab-delimited, `-` for an absent field):

| Record | Fields | Meaning |
|---|---|---|
| `GEN` | path, ts or go, glob or header or both, names=N | A generated contract file and how it was detected |
| `MIRROR` | gen-path, name, handwritten-path:line, excerpt | A handwritten declaration whose name the generated contract also defines |
| `STALECOMMENT` | path:line, nearest-mirror-name, excerpt | Predates / not-regenerated / hand-written-here language near a mirror |
| `OKGEN` | path, names=N, mirrors=0 | A generated file with zero handwritten twins |
| `SUMMARY` | gen_files, names, mirrors, stale_comments, skipped | Always printed; `gen_files=0` is the N/A shape |

Detection: a file is a generated contract by name (`*.generated.*`, `*.gen.go`,
`*.gen.ts`, `*_gen.*`, `/generated/`, `*/types/api.*`) or by header
(`auto-generated`, `DO NOT EDIT`, `do not make direct changes`,
`Code generated by`). Declared names come from `export interface|type|enum|class`
in TS, from openapi-typescript `schemas:`-block keys (which export no
per-schema symbols), and from `type Name struct|interface` in Go.

Judgment:

- A `MIRROR` in a test file is often a deliberate fixture: keep the record, let
  confidence scoring weigh the path.
- A `MIRROR` plus a `STALECOMMENT` naming the generated file is the incident
  shape: a client hand-declaring `SearchResult` beside a comment claiming the
  committed generated types "predate that addition", while the generated file
  contains the schema at HEAD. That pairing is HIGH minimum, CRITICAL when the
  diff touched either side.
- Escalate when the generated header says "do not make direct changes": the
  project already declared the generated file authoritative.
- Generic names (`Error`, `Config`) produce low-value mirrors; the scanner
  deliberately has no stoplist, so drop them at the confidence filter, not by
  editing the scanner.
...
### `[docs/status]` `[docs/live-value]` docs-drift.sh records

`docs-drift.sh` scans the COMPLETE contents of changed markdown, never just the
hunks, because the contradiction is usually between an added banner and an
untouched line forty lines below it. It also checks nearby classifiers (same
directory, repo root, and any doc that names the changed file), bounded and
capped, with every cap announced via `NOTE`.

Record grammar (tab-delimited, `-` for an absent field):

| Record | Fields | Meaning |
|---|---|---|
| `AUTHORITY` | source-path, build or version, value | A discovered authoritative value |
| `DOC` | path, status-bearing or prose, lines=, markers= | Per-file classification |
| `MARKER` | path:line, class, excerpt | Evidence: hist, active, imperative, checkbox, canonical |
| `STALE` | path:line, found-floor, authority-value, authority-source, section, excerpt | A live floor below the authority. A finding |
| `INDEXED` | classifier-path:line, doc-path, historical or live, excerpt | How another doc classifies this one |
| `CONTRA` | status-conflict or index-conflict forms | A lifecycle contradiction. A finding |
| `OKDOC` | path, markers=, stale=0, contra=0 | Scanned clean; absent means NOT scanned |
| `SKIP` / `NOTE` / `SUMMARY` | | Skips, caps, and the always-printed totals |

Authority precedence for `[docs/live-value]`: tracked scalar files
(`BUILD_NUMBER`, `VERSION`, `SHIPPED_VERSION`) and `package.json` /
`plugin.json` versions, then a Makefile `BUILD_NUM :=` variable (a
`$(shell cat FILE)` right-hand side resolves through the named file), then
`project.pbxproj` `CURRENT_PROJECT_VERSION`. Build (integer) and version
(semver) are separate classes; disagreeing build authorities all get
`AUTHORITY` rows plus a `NOTE authority-disagree`, and comparison uses the
max. No authority of the matching class means no `STALE` is ever guessed.

The four deterministic suppressions, in order, first match wins:

1. **Canonical reference**: the line names a discovered authority file as a
   whole token, or contains `$(cat ...)` of one. Emitted as `MARKER canonical`;
   this is the drift-resistant shape to recommend in fixes.
2. **Dated evidence**: an ISO date or "session of" on the line marks it a
   record of what WAS true, not an instruction.
3. **Historical section**: the nearest heading says historical, archive,
   snapshot, superseded, execution record, changelog, or post-mortem.
4. **Wholly-historical file**: historical markers present and zero active
   markers. The whole file is a record; old numbers inside it are correct.

Judgment: `CONTRA` and `STALE` are deterministic findings; carry them unless
the confidence scorer can show the record misread the doc. `MARKER` and
`INDEXED` are evidence with the FRESHNESS `REF`-polarity discipline: a hist
marker in a doc that consistently reads as an archive is fine, and an active
marker alone is fine; it is the combination, or the disagreement with an
index, that the script already promotes to `CONTRA`. A `STALE` whose section
context looks live but whose surrounding prose reads historical is worth one
re-read before reporting: quote the line, the authority value, and the
authority source either way.
