# apresai-skills repository guidance

Guidance for coding agents (Claude Code, Codex) working in this repository. The editable original is `CLAUDE.md`; `AGENTS.md` is its byte-identical mirror for tools that read that filename.
...
- Any PR that touches `CLAUDE.md` refreshes the mirror in the same commit:
  `cp CLAUDE.md AGENTS.md`. `make validate-mirrors` fails the suite otherwise.
  An earlier Codex-generated mirror find-replaced "claude" inside literal
  paths (`.claude-plugin/` became `.Codex-plugin/`), which is why the mirror
  is a byte copy, never a reworded twin.
